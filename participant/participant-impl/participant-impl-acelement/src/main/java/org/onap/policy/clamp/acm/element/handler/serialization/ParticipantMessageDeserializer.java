...
public class ParticipantMessageDeserializer implements Deserializer<ElementMessage> {

    private static final ObjectMapper MAPPER = MapperFactory.createJsonMapper();

    @Override
    public ElementMessage deserialize(final String topic, final byte[] data) {
        try {
            return MAPPER.readValue(new String(data), ElementMessage.class);
