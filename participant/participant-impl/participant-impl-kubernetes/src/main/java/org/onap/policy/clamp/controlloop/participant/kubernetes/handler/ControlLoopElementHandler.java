...
public class ControlLoopElementHandler implements ControlLoopElementListener {
    ...
    private static Map<String, ChartInfo> chartMap = new HashMap<>();
    ...
    public void controlLoopElementUpdate(ControlLoopElement element,
        ...
        for (Map.Entry<String, ToscaNodeTemplate> nodeTemplate: controlLoopDefinition.getToscaTopologyTemplate()
            ...
            if (element.getDefinition().getName().equals(nodeTemplate.getKey())) {
                if (nodeTemplate.getValue().getProperties().containsKey("chart")) {
                    LinkedTreeMap chartData = (LinkedTreeMap) nodeTemplate.getValue().getProperties().get("chart");

                    logger.info("Installation request received for the Helm Chart {} ", chartData);
                    ChartInfo chart = new ChartInfo(String.valueOf(chartData.get("release_name")),
