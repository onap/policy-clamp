...
public class HelmClient {
     ...
     */
    public void installChart(ChartInfo chart) throws IOException, ServiceException {
    ...
    private boolean checkNamespaceExists(String namespace) throws ServiceException, IOException {
    ...
    private String verifyOutput(String output, String value) throws IOException {
        String result = null;
    ...
    private ProcessBuilder prepareVerifyRepoCommand(HelmRepository repo) {
        List<String> helmArguments = new ArrayList<>(List.of("sh", "-c", "helm repo ls | grep " + repo.getRepoName()));
        return new ProcessBuilder().command(helmArguments);
    }

    private ProcessBuilder prepareVerifyNamespaceCommand(String namespace) {
        List<String> helmArguments = new ArrayList<>(List.of("sh", "-c", "kubectl get ns | grep " + namespace));
        return new ProcessBuilder().command(helmArguments);
    }
