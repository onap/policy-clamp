...
public class ChartStore {
    ...
    public static final String localChartDir = "/var/helm-manager/database";
    ...
    public File getHelmChartFile(ChartInfo chart) {
        Path appPath = getAppPath(chart.getChartName(), chart.getVersion());
    ...
    public synchronized ChartInfo saveChart(ChartInfo chartInfo, MultipartFile chartFile, MultipartFile overrideFile)
        ...
        Path appPath = getAppPath(chartInfo.getChartName(), chartInfo.getVersion());
     ...
     * @throws ServiceException incase of error
     */
    public synchronized ChartInfo getChart(String name, String version) throws ServiceException {
        ChartInfo chart = localChartMap.get(key(name, version));
        if (chart == null) {
            throw new ServiceException("Chart not found: " + name);
        }
        return chart;
    }
    ...
    private void storeChartInFile(ChartInfo chart) {
        try (PrintStream out = new PrintStream(new FileOutputStream(getFile(chart)))) {
    ...
    private File getFile(ChartInfo chart) {
        String appPath = getAppPath(chart.getChartName(), chart.getVersion()).toString();
        return Path.of(appPath, INFO_FILE_NAME).toFile();
    }

    private synchronized void restoreFromDatabase() {
        try {
            Files.createDirectories(Paths.get(localChartDir));
            restoreFromDatabase(new File(localChartDir));
        } catch (IOException e) {
    ...
    private synchronized void reset() {
        try {
            FileSystemUtils.deleteRecursively(Path.of(localChartDir));
        } catch (IOException e) {
