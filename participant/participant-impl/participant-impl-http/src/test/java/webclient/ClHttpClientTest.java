...
class ClHttpClientTest {
    ...
    private ControlLoopElementHandler handler = new ControlLoopElementHandler();

    @AfterEach
    void clearClResponseMap() {
        handler.getRestResponseMap().clear();
    }
    ...
    void test_validRequest() {
        ...
        ClHttpClient client = new ClHttpClient(configRequest, handler.getRestResponseMap());
        assertDoesNotThrow(client::run);
        assertThat(handler.getRestResponseMap()).hasSize(2).containsKey(commonTestData
            .restParamsWithGet().getRestRequestId());

        Pair<Integer, String> restResponseMap = handler.getRestResponseMap().get(commonTestData
            .restParamsWithGet().getRestRequestId());
    ...
    void test_invalidRequest() {
        ...
        assertThat(handler.getRestResponseMap()).hasSize(2).containsKey(commonTestData
            .restParamsWithGet().getRestRequestId());
        Pair<Integer, String> responseMap = handler.getRestResponseMap()
            .get(commonTestData.restParamsWithInvalidPost().getRestRequestId());
        assertThat(responseMap.getKey()).isEqualTo(404);
    }
}
