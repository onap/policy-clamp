...
class ClHttpClientTest {
    ...
    void clearClResponseMap() {
        ControlLoopElementHandler.getRestResponseMap().clear();
    }
    ...
    void test_validRequest() {
        ...
        ClHttpClient client = new ClHttpClient(configRequest);
        assertDoesNotThrow(client::run);
        assertThat(ControlLoopElementHandler.getRestResponseMap()).hasSize(2).containsKey(commonTestData
            .restParamsWithGet().getRestRequestId());

        Map<Integer, String> restResponseMap = ControlLoopElementHandler.getRestResponseMap().get(commonTestData
            .restParamsWithGet().getRestRequestId());
        assertThat(restResponseMap).containsKey(200);
    }
    ...
    void test_invalidRequest() {
        ...
        assertThat(ControlLoopElementHandler.getRestResponseMap()).hasSize(2).containsKey(commonTestData
            .restParamsWithGet().getRestRequestId());
        Map<Integer, String> responseMap = ControlLoopElementHandler.getRestResponseMap()
            .get(commonTestData.restParamsWithInvalidPost().getRestRequestId());
        assertThat(responseMap).containsKey(404);
    }
}
