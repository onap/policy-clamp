...
public class ParticipantSyncMessageListener {
    ...
    public void onTopicEvent(final ParticipantSync participantSync) {
        if (participantHandler.appliesTo(participantSync)) {
            logEvent(participantSync.toString());
            participantHandler.handleParticipantSync(participantSync);
        }
    }
    ...
    public void onTopicEvent(final ParticipantRegisterAck participantRegisterAck) {
        if (participantHandler.appliesTo(participantRegisterAck)) {
            logEvent(participantRegisterAck.toString());
            participantHandler.handleParticipantRegisterAck(participantRegisterAck);
        }
    }
    ...
    public void onTopicEvent(final ParticipantStatusReq participantStatusReq) {
        if (participantHandler.appliesTo(participantStatusReq)) {
            logEvent(participantStatusReq.toString());
            participantHandler.handleParticipantStatusReq(participantStatusReq);
        }
    }
    ...
    private void logEvent(final String message) {
        NetLoggerUtil.log(NetLoggerUtil.EventType.IN, "KAFKA", syncTopic, message);
    }
}
