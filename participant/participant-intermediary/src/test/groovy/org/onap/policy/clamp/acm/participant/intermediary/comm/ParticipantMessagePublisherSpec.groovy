...
class ParticipantMessagePublisherSpec extends Specification {
    ...
    def "should send ParticipantStatus with participantId as partition key"() {
        given:
    ...
    def "should send ParticipantRegister with participantId as partition key"() {
        given:
    ...
    def "should send ParticipantDeregister with participantId as partition key"() {
        given:
    ...
    def "should send ParticipantPrimeAck with compositionId as partition key"() {
        given:
    ...
    def "should send AutomationCompositionDeployAck with compositionId as partition key"() {
        given:
    ...
    def "should send ParticipantReqSync with participantId as partition key"() {
        given:
    ...
    def "should send without partition key when message has no IDs"() {
        given:
    ...
    def "should log warning and not throw when send fails"() {
        given:
        def message = new ParticipantRegister(participantId: PARTICIPANT_ID)
        ...
        then:
        1 * kafkaTemplate.send(TOPIC, PARTICIPANT_ID.toString(), message) >> {
