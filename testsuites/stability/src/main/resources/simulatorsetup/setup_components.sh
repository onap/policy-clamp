...
# get the CLAMP_HOST from the command line
read -p 'CLAMP_HOST : ' CLAMP_HOST

# setting hostname to environment variable
echo "export API_HOST=`hostname -I | awk '{print $1}'`" >> ~/.bashrc
echo "export CLAMP_HOST=$CLAMP_HOST" >> ~/.bashrc
source ~/.bashrc
...
cd ../policy-models-sim-participant
${WORK_DIR}/maven/apache-maven-3.3.9/bin/mvn clean install -DskipTests  --settings ${WORK_DIR}/oparent/settings.xml
bash ./src/main/package/docker/docker_build.sh
...
docker run \
    -p 3306:3306 \
    -v ${DIR}/config/db:/docker-entrypoint-initdb.d \
    --name mariadb  \
    --env-file ${DIR}/config/db/db.conf \
    -d \
    --rm mariadb:10.2.14 \
...
sleep 10
docker run --link message-router:message-router -d --name participant-simulator --rm participant/simulator:latest
