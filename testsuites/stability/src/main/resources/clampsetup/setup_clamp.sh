...
CLAMP_IMAGE=policy-pap:${POLICY_CLAMP_VERSION_EXTRACT:0:3}-SNAPSHOT-latest

docker run \
	-p 9090:9090 \
	-p 6969:6969 \
	-e "CLAMP_HOST=${CLAMP}" \
	-v ${DIR}/config/pap/bin/policy-pap.sh:/opt/app/policy/pap/bin/policy-pap.sh \
	-v ${DIR}/config/pap/etc/defaultConfig.json:/opt/app/policy/pap/etc/defaultConfig.json \
