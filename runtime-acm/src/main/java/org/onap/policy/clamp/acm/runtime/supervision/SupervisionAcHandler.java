...
public class SupervisionAcHandler {
    ...
    public void deploy(AutomationComposition automationComposition, AutomationCompositionDefinition acDefinition) {
        if (ErrorStatus.FAILED.equals(automationComposition.getErrorStatus())) {
        ...
        automationComposition.setErrorStatus(ErrorStatus.NO_ERROR);
    ...
    private void setAcElementStateInDb(AutomationCompositionDeployAck automationCompositionAckMessage) {
        ...
        var updated = updateState(automationComposition.get(),
                ...
                automationCompositionAckMessage.getErrorStatus());
        if (updated) {
            automationCompositionProvider.updateAutomationComposition(automationComposition.get());
        }
    }
    ...
    private boolean updateState(AutomationComposition automationComposition,
            Set<Map.Entry<UUID, AcElementDeployAck>> automationCompositionResultSet, ErrorStatus errorStatus) {
        var updated = false;
        var elementInErrors = ErrorStatus.FAILED.equals(errorStatus);
        boolean inProgress = ErrorStatus.NO_ERROR.equals(automationComposition.getErrorStatus());
        ...
        if (elementInErrors && inProgress) {
            automationComposition.setErrorStatus(ErrorStatus.FAILED);
        }
        return updated;
    }
}
