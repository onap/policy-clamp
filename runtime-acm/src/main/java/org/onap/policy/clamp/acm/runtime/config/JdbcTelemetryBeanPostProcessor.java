...
public class JdbcTelemetryBeanPostProcessor implements BeanPostProcessor, ApplicationContextAware {
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (bean instanceof HikariDataSource hikariDataSource) {
            ...
            var tracingDataSource = JdbcTelemetry.create(openTelemetry).wrap(hikariDataSource);
            return new DelegatingDataSource(hikariDataSource) {
                @Override
                public Connection getConnection() throws SQLException {
                    if (Span.current().getSpanContext().isValid()) {
                        return tracingDataSource.getConnection();
                    }
                ...
                public Connection getConnection(String username, String password) throws SQLException {
                    if (Span.current().getSpanContext().isValid()) {
                        return tracingDataSource.getConnection(username, password);
                    }
