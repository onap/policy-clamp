...
class SupervisionMessagesSpec extends Specification {
    ...
    def "send ParticipantPrime for priming - #scenario"() {
        given:
        ...
        where:
        scenario        | acmDefinition                    | participantProvider
        "commissioned"  | {
            ...
            def acmDef = new AutomationCompositionDefinition(
                    compositionId: UUID.randomUUID(),
                    serviceTemplate: serviceTemplate,
                    state: AcTypeState.COMMISSIONED)
            acmDef.elementStateMap = AcmUtils.createElementStateMap(
                    AcmUtils.extractAcElementsFromServiceTemplate(serviceTemplate, CommonTestData.TOSCA_ELEMENT_NAME),
                    AcTypeState.COMMISSIONED)
            acmDef
        }()             | Mock(ParticipantProvider) { getSupportedElementMap() >> [
                    (new ToscaConceptIdentifier("org.onap.policy.clamp.acm.PolicyAutomationCompositionElement", "1.0.0")): UUID.randomUUID(),
                    (new ToscaConceptIdentifier("org.onap.policy.clamp.acm.K8SMicroserviceAutomationCompositionElement", "1.0.0")): UUID.randomUUID(),
                    (new ToscaConceptIdentifier("org.onap.policy.clamp.acm.HttpAutomationCompositionElement", "1.0.0")): UUID.randomUUID()
            ] }
        "primed"        | buildAcmDefinition(AcTypeState.PRIMED)  | Mock(ParticipantProvider)
    }
