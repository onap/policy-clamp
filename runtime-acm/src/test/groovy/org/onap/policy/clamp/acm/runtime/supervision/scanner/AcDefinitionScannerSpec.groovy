...
class AcDefinitionScannerSpec extends Specification {
    ...
    def "scan definition in FAILED state should not update provider"() {
        given:
        ...
        def acDefinition = createAcDefinition(AcTypeState.PRIMING, StateChangeResult.FAILED)

        when:
        scanner.scanAutomationCompositionDefinition(acDefinition, new UpdateSync())

        then:
        0 * acDefinitionProvider.updateAcDefinitionState(_)
        0 * participantSyncPublisher.sendSync(_, _)
    }
