...
class CommissioningProviderSpec extends Specification {
    ...
    def setup() {
        ...
        participantPrimePublisher = Mock(ParticipantPrimePublisher)
    }
    ...
    def "should fail with expected error when #scenario"() {
        given: "a composition with the specified conditions"
        def compositionId = UUID.randomUUID()
        acProvider.getAcInstancesByCompositionId(compositionId) >> instances
        acProvider.getAcInstancesByTargetCompositionId(compositionId) >> targetInstances

        if (state != null) {
            def acmDefinition = CommonTestData.createAcDefinition(loadServiceTemplate(), state)
            acmDefinition.compositionId = compositionId
            acDefinitionProvider.getAcDefinition(compositionId) >> acmDefinition
        }

        def provider = createProvider(params)

        when: "attempting the operation"
        operation(provider, compositionId)

        then: "a PfModelRuntimeException should be thrown with the expected message"
        def ex = thrown(PfModelRuntimeException)
        ex.message.contains(expectedMessage)

        where:
        scenario | instances | targetInstances | state | params | operation || expectedMessage

        "priming with existing instances" |
                [new AutomationComposition()] | [] | null |
                CommonTestData.getTestParamaterGroup() |
                { p, id -> p.compositionDefinitionPriming(id, new AcTypeStateUpdate()) } ||
                ERROR_MESSAGES["primingWithInstances"]

        "priming with target composition instances" |
                [] | [new AutomationComposition()] | null |
                CommonTestData.getTestParamaterGroup() |
                { p, id -> p.compositionDefinitionPriming(id, new AcTypeStateUpdate()) } ||
                ERROR_MESSAGES["primingWithTargetComposition"]

        "updating with existing instances" |
                [new AutomationComposition()] | [] | null |
                Mock(AcRuntimeParameterGroup) |
                { p, id -> p.updateCompositionDefinition(id, new ToscaServiceTemplate()) } ||
                ERROR_MESSAGES["updateWithInstances"]

        "updating a PRIMED definition" |
                [] | [] | AcTypeState.PRIMED |
                Mock(AcRuntimeParameterGroup) |
                { p, id -> p.updateCompositionDefinition(id, loadServiceTemplate()) } ||
                ERROR_MESSAGES["updateNotCommissioned"]
    }
