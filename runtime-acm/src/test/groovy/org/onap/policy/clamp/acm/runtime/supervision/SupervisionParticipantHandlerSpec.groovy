...
class SupervisionParticipantHandlerSpec extends Specification {
    ...
    def "deregister '#desc' should send ack"() {
        ...
        desc             | msgParams
        "no replicaId"   | [:]
        "with replicaId" | [replicaId: REPLICA_ID]

        lookupId = msgParams.replicaId ?: PARTICIPANT_ID
        replicaOpt = msgParams.replicaId
                ? Optional.of(CommonTestData
                        .createParticipantReplica(REPLICA_ID))
                : Optional.empty()
        deleteCount = msgParams.replicaId ? 1 : 0
    }
