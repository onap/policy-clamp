...
class PhaseScannerSpec extends Specification {
    ...
    def "scan deploying AC with next phase should send deploy message"() {
        ...
        then:
        1 * acDeployPublisher.send(_ as AutomationComposition, _ as int, _ as boolean, _ as AutomationCompositionDefinition)
    }
    ...
    def "scan deploying AC with null start phase should not send message"() {
        ...
        then:
        0 * acDeployPublisher.send(_ as AutomationComposition, _ as int, _ as boolean, _ as AutomationCompositionDefinition)
    }

    def "scan undeploying AC with compositionTargetId should send state change message with target definition"() {
        given:
        def ac = buildUndeployingAcWithTarget(COMPOSITION_ID, INSTANCE_ID)
        def targetDefinition = new AutomationCompositionDefinition()
