...
import {
    pollUntil,
    pollPriming,
    pollStateChangeResult,
    pollDeployStatus,
    pollInternalStateElementsRuntime,
    pollUninstantiated,
    verifyPriming,
    verifyDeployStatus,
...
export default function () {
    ...
    group('PrimeACDefinitionsSimple', () => {
        ...
        pollUntil(
            () => pollPriming(ACM_HOST, compositionId, 'PRIMED'),
    ...
    group('FailDeployAutomationCompositionSimple', () => {
        ...
        pollUntil(
            () => pollStateChangeResult(ACM_HOST, compositionId, instanceId, 'FAILED'),
            120000,  // 2 min
            5000     // 5 sec
        );
        verifyDeployStatus(ACM_HOST, compositionId, instanceId, 'DEPLOYING', 'FAILED');

        verifyCompositionParticipantSim(SIM1_HOST, compositionId, '"name":"onap.policy.clamp.ac.element.Sim_AutomationCompositionElement"');
    });
    ...
    group('UnDeployAutomationCompositionSimple', () => {
        ...
        pollUntil(
            () => pollDeployStatus(ACM_HOST, compositionId, instanceId, 'UNDEPLOYING'),
            60000,   // 1 min
            5000
        );

        pollUntil(
            () => pollInternalStateElementsRuntime(ACM_HOST, compositionId, instanceId, 'UNDEPLOYING', ELEMENT_ID),
            60000,   // 1 min
            5000
        );

        pollUntil(
            () => pollDeployStatus(ACM_HOST, compositionId, instanceId, 'UNDEPLOYED'),
            180000,  // 3 min
            5000
        );
    ...
    group('UnInstantiateAutomationCompositionSimple', () => {
        ...
        pollUntil(
            () => pollUninstantiated(ACM_HOST, compositionId),
            60000,   // 1 min
            5000
        );
    ...
    group('DeleteACDefinitionSimple', () => {
        ...
        pollUntil(
            () => pollPriming(ACM_HOST, compositionId, 'COMMISSIONED'),
            120000,  // 2 min
            5000
        );
