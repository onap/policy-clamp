...
/**
 * Poll-safe probe — no check() calls, safe to use inside pollUntil.
 * @param {string} acmHost
 * @param {string} compositionId
 * @param {string} expectedState
 * @returns {boolean}
 */
function probePriming(acmHost, compositionId, expectedState) {
    const resp = get(acmHost, `/onap/policy/clamp/acm/v2/compositions/${compositionId}`, clampAuth());
    if (resp.status !== 200) return false;
    const body = JSON.parse(resp.body);
    return body.stateChangeResult === 'NO_ERROR' && body.state === expectedState;
}

/**
 * Final assertion — records check() results. Call after pollUntil.
 * @param {string} acmHost
 * @param {string} compositionId
...
/**
 * Poll-safe wrapper for use inside pollUntil.
 * @param {string} acmHost
 * @param {string} compositionId
 * @param {string} expectedState
 * @returns {boolean}
 */
export function pollPriming(acmHost, compositionId, expectedState) {
    return probePriming(acmHost, compositionId, expectedState);
}
...
/**
 * Poll-safe probe for deployState.
 */
function probeDeployStatus(acmHost, compositionId, instanceId, expectedDeployState) {
    const resp = get(
        acmHost,
    ...
    if (resp.status !== 200) return false;
    const body = JSON.parse(resp.body);
    return body.deployState === expectedDeployState;
}
...
export function verifyDeployStatus(acmHost, compositionId, instanceId, expectedDeployState,
        expectedStateChangeResult = 'NO_ERROR') {
    const resp = get(
        acmHost,
        `/onap/policy/clamp/acm/v2/compositions/${compositionId}/instances/${instanceId}`,
        clampAuth()
    ...
    return check(resp, {
        ...
        [`deployStatus: stateChangeResult is ${expectedStateChangeResult}`]:     r => JSON.parse(r.body).stateChangeResult === expectedStateChangeResult,
        [`deployStatus: deployState is ${expectedDeployState}`]:                 r => JSON.parse(r.body).deployState === expectedDeployState,
    });
}

export function pollDeployStatus(acmHost, compositionId, instanceId, expectedDeployState) {
    return probeDeployStatus(acmHost, compositionId, instanceId, expectedDeployState);
}
...
/**
 * Poll-safe probe for stateChangeResult.
 */
function probeStateChangeResult(acmHost, compositionId, instanceId, expectedResult) {
    const resp = get(
        acmHost,
        `/onap/policy/clamp/acm/v2/compositions/${compositionId}/instances/${instanceId}`,
        clampAuth()
    ...
    if (resp.status !== 200) return false;
    return JSON.parse(resp.body).stateChangeResult === expectedResult;
}
...
export function pollStateChangeResult(acmHost, compositionId, instanceId, expectedResult) {
    return probeStateChangeResult(acmHost, compositionId, instanceId, expectedResult);
}
...
/**
 * Poll-safe probe for element internal state.
 */
function probeInternalStateElementsRuntime(acmHost, compositionId, instanceId, expectedDeployState, elementId) {
    const resp = get(
        acmHost,
        `/onap/policy/clamp/acm/v2/compositions/${compositionId}/instances/${instanceId}`,
        clampAuth()
    ...
    const body = JSON.parse(resp.body);
    return body.deployState === expectedDeployState &&
        body.elements?.[elementId]?.outProperties?.InternalState === expectedDeployState;
}
...
export function pollInternalStateElementsRuntime(acmHost, compositionId, instanceId, expectedDeployState, elementId) {
    return probeInternalStateElementsRuntime(acmHost, compositionId, instanceId, expectedDeployState, elementId);
}
...
/**
 * Poll-safe probe for uninstantiated state.
 */
function probeUninstantiated(acmHost, compositionId) {
    const resp = get(
        acmHost,
    ...
    if (resp.status !== 200) return false;
    return JSON.parse(resp.body).automationCompositionList?.length === 0;
}
...
export function pollUninstantiated(acmHost, compositionId) {
    return probeUninstantiated(acmHost, compositionId);
}
...
/**
 * Mirrors Robot's VerifyCompositionParticipantSim:
 *   GET /onap/policy/simparticipant/v2/compositiondatas/${compositionId}
 *   assert status 200 and body contains the full element type string.
 *
 * @param {string} simHost
 * @param {string} compositionId
 * @param {string} elementType  full element type string, e.g.
 *                              "{'name': 'onap.policy.clamp.ac.element.Sim_AutomationCompositionElement', 'version': '1.2.3'}"
 * @returns {boolean}
 */
export function verifyCompositionParticipantSim(simHost, compositionId, elementType) {
    const resp = get(simHost, `/onap/policy/simparticipant/v2/compositiondatas/${compositionId}`, participantAuth());
    return check(resp, {
        'compositionSim: status 200':                    r => r.status === 200,
        [`compositionSim: contains ${elementType}`]:     r => r.body.includes(elementType),
    });
}
