...
if [ $OPERATION == "install" ]; then
    ...
    if [ "${?}" -eq 0 ]; then
        ...
        source ${WORKSPACE}/csit/resources/compose/get-k8s-versions.sh
        if [ $LOCALIMAGE == "true" ]; then
            echo "loading local image"
            source ${WORKSPACE}/csit/resources/compose/get-versions.sh
            ${WORKSPACE}/csit/resources/compose/loaddockerimage.sh
        fi
