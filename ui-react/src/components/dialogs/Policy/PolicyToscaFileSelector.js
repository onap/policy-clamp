                ...
                fileReader.onload = (content) => {
                    PolicyService.sendNewPolicyModel(atob(content.target.result.split(",")[1])).then(respModelCreate => {
                          if (typeof(respModelCreate) === "undefined") {
                            ...
                            this.setState(state => {
                                const alertMessages = [...state.alertMessages,(<Alert variant="danger"><Alert.Heading>{file.name}</Alert.Heading><p>Policy Tosca Model Creation Failure</p><hr/><p>Type: {file.type}</p><p>Size: {file.size}</p></Alert>)];
                                return {
                                    alertMessages
                                };
                            });
                            ...
                            this.setState(state => {
                                const alertMessages = [...state.alertMessages,(<Alert variant="success"><Alert.Heading>{file.name}</Alert.Heading><p>Policy Tosca Model Created Successfully</p><hr/><p>Type: {file.type}</p><p>Size: {file.size}</p></Alert>)];
                                return {
                                    alertMessages
                                };
                            });
			...
			<ModalStyled size="lg" show={this.state.show} onHide={this.handleClose} backdrop="static" keyboard={false} >
				...
				<Modal.Body>
					<Form.Group as={Row} controlId="formPlaintextEmail">
						<Col sm="10">
                           <input type="file" multiple id="fileUploadButton" style={{ display: 'none' }} onChange={this.onFileChange} />
