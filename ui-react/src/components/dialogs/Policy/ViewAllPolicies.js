...
export default class ViewAllPolicies extends React.Component {
  state = {
    ...
    handlePdpGroupChange(e) {
        this.state.policiesListData[this.state.selectedRow]["pdpGroupInfo"] = {"pdpGroup":e.value};
        //this.setState({ policiesListData: this.policiesListData });
    }
    ...
    renderPdpGroupDropBox(dataRow) {
        ...
        if (typeof dataRow.supportedPdpGroups !== "undefined") {
            optionItems.push(...dataRow["supportedPdpGroups"].map(pdpGroup =>
            {
                let optionArray = [];
               for (const pdpSubGroup of Object.values(pdpGroup)) {
                    optionArray.push({ label: Object.keys(pdpGroup)[0]+"/"+pdpSubGroup,
