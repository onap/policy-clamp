...
public class PdpGroupsAnalyzer {
     ...
     */
    static private JsonObject getSupportedSubgroups(PdpGroup pdpGroup, String policyType, String version) {
        if (!PdpState.ACTIVE.equals(pdpGroup.getPdpGroupState())) {
            return null;
        }
        JsonArray supportedSubgroups = new JsonArray();
        for (PdpSubGroup subGroup : pdpGroup.getPdpSubgroups()) {
            for (ToscaPolicyTypeIdentifier policyTypeIdentifier:subGroup.getSupportedPolicyTypes()) {
                if (policyType.matches(policyTypeIdentifier.getName().replace(".","\\.").replace("*",".*")) && version.equals(policyTypeIdentifier.getVersion())) {
     ...
     */
    static public void updatePdpGroup(List<PolicyModel> policyModelsList, PdpGroups pdpGroups) {
        for (PolicyModel policyModel : policyModelsList) {
            JsonArray supportedPdpGroups = new JsonArray();
            for (PdpGroup pdpGroup : pdpGroups.getGroups()) {
                JsonObject supportedPdpGroup = PdpGroupsAnalyzer.getSupportedSubgroups(pdpGroup,
